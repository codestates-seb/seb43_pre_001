...
import { setAllTags, setDeleteTag, setDiscardTags, setTagsFocus } from '../../reducer/askSlice';
...
const HashTagsWrapper = styled(TagBoxStyle)`
  ...
  box-shadow: ${(props) => {
    if (props.tagsFocus) {
      return props.tagsErrorMsg ? '0 0 0 4px hsl(0deg 46% 92%)' : '0 0 0 4px hsl(206deg 65% 91%)';
    }
  }};
`;
...
function InputTags({ defaultValue = null }) {
  ...
  let { allTags, tagsFocus, discardTags } = useSelector((state) => state.ask);
  ...
  let validationHandler = () => {
    if (!allTags?.length) {
      isTagsValid = false;
      setTagsErrorMsg('Please enter at least one tag; see a list of popular tags.');
  ...
  useEffect(() => validationHandler(), [allTags]);
  useEffect(() => {
    if (discardTags) {
      resetInputTags();
    }
  ...
  const makeTag = (value) => {
    ...
    svgPath.setAttribute('d', 'M12 3.41L10.59 2 7 5.59 3.41 2 2 3.41 5.59 7 2 10.59 3.41 12 7 8.41 10.59 12 12 10.59 8.41 7z');
  ...
  let pushTag = (e) => {
    if (e.key === 'Enter' && !allTags.includes(currentTag) && !currentTag.split('').every((el) => el === ' ')) {
      makeTag(currentTag);
      setCurrentTag('');
    }
  };
